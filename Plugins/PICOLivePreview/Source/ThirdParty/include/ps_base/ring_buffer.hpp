...
/// @brief Copied from https://github.com/jnk0le/Ring-Buffer
 ...
 public:
  ...
  bool Remove() {
    ...
    if (tmp_tail == head.load(std::memory_order_relaxed))
      ...
      tail.store(++tmp_tail, index_release_barrier);  // release in case data
                                                      // was loaded/used before
  ...
  /*!
   * \brief Gets the last element in the buffer on produced side
   *
   * It is safe to use and modify item contents only on produced side
   *
  ...
  T* Head() {
    ...
    if (tmp_head == tail.load(index_acquire_barrier))
      return nullptr;
    else
  ...
  /*!
   * \brief Gets the n'th storage space on consumed side
   *
   * \warning Use this only when you really know what you do!!!!!
   *
   * \param index Item offset starting from storage on the consumed side
   * \return Pointer to requested element, nullptr if index exceeds storage
   * count
   */
  T* StorageAt(size_t index) {
    index_t tmp_tail = tail.load(std::memory_order_relaxed);
